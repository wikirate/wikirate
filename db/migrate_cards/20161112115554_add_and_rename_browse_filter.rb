...
class AddAndRenameBrowseFilter < Card::Migration
  ...
  def update_existing_filter_cards
    %w(metric topic company).each do |name|
      ...
      card.update_attributes! name: "browse #{name} filter", codename: "browse_#{name}_filter",
                              update_referers: true
    end
  end

  def create_new_filter_cards
    ensure_card "browse note filter", codename: "browse_note_filter", type_id: Card::SearchTypeID
    ensure_card "browse source filter", codename: "browse_source_filter" , type_id: Card::SearchTypeID
  end
end
