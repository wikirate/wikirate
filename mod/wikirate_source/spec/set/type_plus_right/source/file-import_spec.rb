describe Card::Set::TypePlusRight::Source::File::Import do
  ...
  describe 'updating metric values' do
    it 'updates correct metric values' do
      ...
      amazon_2015_metric_value_card = Card['Access to Nutrition Index+Marketing Score+Amazon.com, Inc.+2015+value']
      apple_2015_metric_value_card = Card['Access to Nutrition Index+Marketing Score+Apple Inc.+2015+value']
      expect(amazon_2015_metric_value_card.content).to eq('369')
      expect(apple_2015_metric_value_card.content).to eq('689')
    end
  end
  describe 'while rendering import view' do
    ...
    it 'shows field correctly' do
      ...
      expect(html).to have_tag('input', with: {
        id: 'is_metric_import_update',value: 'true',type: 'hidden'
      })
      expect(html).to have_tag('table', with: {class: 'import_table'}) do
        with_tag 'tr' do
          with_tag 'input', with: {
            type: 'checkbox',value: '43',id: 'metric_values_Cambridge_'
          }
          with_tag 'td',text: 'Cambridge'
          with_tag 'td',text: 'none'
          with_tag 'td' do
            with_tag 'input', with: {
              type: 'text', name: 'corrected_company_name[Cambridge]'
            }
          end
        end
        with_tag 'tr' do
          with_tag 'input', with: {
            checked: 'checked',type: 'checkbox',value: '9',
        ...
        with_tag 'tr' do
          with_tag 'input', with: {
            checked: 'checked',type: 'checkbox',value: '62',
        ...
        with_tag 'tr' do
          with_tag 'input', with: {
            checked: 'checked',type: 'checkbox',value: '33',
