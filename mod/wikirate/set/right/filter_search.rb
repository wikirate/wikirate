def get_query params={}
  ...
  params[:query] = Card.tag_filter_query(
    filter_words, search_args, %w(tag company topic)
  )
  super(params)
end
...
format :html do
  ...
  view :no_search_results do |args|
    %(<div class="search-no-results">No result</div>)
  end

  view :filter_form do |args|
    # args[:buttons] = button_tag 'Filter',
    #   :class=>'submit-button', :disable_with=>'Filtering'
    content = output([
  ...
  view :sort_formgroup do |args|
    select_filter 'sort', options_for_select(
  ...
  view :cited_formgroup do |args|
    select_filter 'cited', options_for_select(
  ...
  view :claimed_formgroup do |args|
    select_filter 'claimed', options_for_select(
      { 'All' => 'all', 'Yes' => 'yes', 'No' => 'no' },
      params[:claimed] || 'all'
    )
  end
  ...
  def multiselect_filter type_name, args
    ...
    multiselect_tag = select_tag(
      type_name, options, multiple: true, class: 'pointer-multiselect'
    )
    formgroup(
      type_name.capitalize, multiselect_tag, class: "filter-input #{type_name}"
    )
  end
end
