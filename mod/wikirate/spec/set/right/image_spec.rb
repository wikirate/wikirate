  ...
  describe "missing view" do
    context "'`type`+missing_image_card' is set" do
      before do
        ...
        @missing_image_card = Card.create! :name=>"user+missing_image_card",:type_id=>Card::ImageID,:image=>file
        @content_card = Card.create! :name=>"Joe, My Son",:content=>"{{joe user+image}}"
      end
      ...
      it "renders based on args' home view" do
        ...
        expect(html).to include(@missing_image_card.format.render_core)
        expect(html).to have_tag("div",:with=>{:class=>"RIGHT-missing_image_card"})
      end 
    end
