  ...
  class RecordQuery
    ...
    module AdvancedFilters
      ...
      def filter_by_related_company_group company_group
        ...
        restrict_to_ids :record_id,
                        Relationship.record_ids_for(metric_card, company_group)
      end

      # # TODO: delete the following after confirming fashionchecker works without it
      #
      # def filter_by_record value
      #   return unless (metric_id = value[:metric_id]&.to_i)
      #   exists = "SELECT * from records AS a2 " \
      #     "WHERE records.company_id = a2.company_id " \
      #     "AND a2.metric_id = ?"
      ...
      #
      # This is ultimately a company restriction, limiting the records to the
      ...
      def filter_by_relationship value
        ...
        exists = "SELECT * FROM relationships AS r " \
          "WHERE records.company_id = r.#{m.company_id_field} " \
