  ...
  context "with fixed company" do
    ...
    describe "#filter_by_metric_keyword" do
      it "finds exact match" do
        expect(search(metric_keyword: %i[esg_topic environment].cardname))
    ...
    specify "#filter_by_topic" do
      expect(search(topic: %i[esg_topic environment].cardname)).to eq ["disturbances in the Force+2001"]
    end
