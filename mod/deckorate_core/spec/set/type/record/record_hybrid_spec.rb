...
RSpec.describe Card::Set::Type::Record, "hybrid" do
  let(:metric) { Card["Jedi+friendliness"] }
  let(:company) { "Death Star" }
  let(:input_metric) { Card["Jedi+deadliness"] }
  let(:calculated_record) { record 1977 }

  def record year=1977
    Card.fetch(metric.name, company, year.to_s).record
  end
  ...
  def input_for_calculation value, year=2010
    create_records input_metric, true do
      Death_Star year.to_s => value.to_s
    end
  end
  ...
  example "initial value" do
    expect(record.overridden_value).to eq nil
    expect(record.value).to eq "0.01"
  end

  example "research calculated value" do
    research_value 5
    expect(record).to have_attributes(overridden_value: "0.01",
                                      value: "5",
  ...
  example "calculate researched value 1" do
    ...
    expect(record(2010)).to have_attributes(value: "5",
  ...
  example "uncalculate researched value" do
    research_value 5
    delete_input 1977
    expect(record).to have_attributes(value: "5",
  ...
  example "unresearch calculated value", as_bot: true do
    ...
    expect(record).to have_attributes(overridden_value: nil,
                                      record_id: nil,
                                      value: "0.01")
  end
end
