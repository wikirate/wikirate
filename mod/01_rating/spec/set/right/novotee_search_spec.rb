describe Card::Set::Right::NovoteeSearch do
  describe "#get_result_from_session" do
    before do
      ...
      @metrics = [get_a_sample_metric, Card.create!(:name=>"Hello+World",:type_id=>Card::MetricID),Card.create!(:name=>"Hello+Boys",:type_id=>Card::MetricID)]
    ...
    it "returns correct results without being voted" do
      ...
      @metrics.each do |metric|
        cached_count = Card.new :name=>"#{metric.name}+#{@company.name}+*cached count"
        cached_count.format.render_core
      end

      metric_novotee_search_card = Card.fetch "#{@company.name}+metric+novotee search"
      search_result = metric_novotee_search_card.format.list_with_no_session_votes
