  ...
  class Calculator
    class Input
      ...
      module Values
        ...
        def fetch_val company_id, year
          return unless @result_cache.has_value? company_id, year

          catch(:cancel_calculation) do
            @input_list.map { |input_item| input_item.value_for company_id, year }
          end
        end

        def yield_value company_id, year
          yield fetch_val(company_id, year), company_id, year
        end
        ...
        def values_for_companies_and_years companies, years, &block
          each_year(years) do |year|
            each_company(companies) do |company_id|
              ...
              yield_value company_id, year, &block
            end
          end
        end
