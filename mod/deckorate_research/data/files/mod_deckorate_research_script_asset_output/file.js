...
(function() {
  ...
  decko.slotReady(function(slot) {
    ...
    if (slot.closest(".research-layout")[0]) {
      ...
      if (btn.length && slot.find("._edit-answer-button").length) {
        btn = btn.clone();
        return slot.find(".button-form-group").append(btn);
      }
    }
  });
  ...
  $(document).ready(function() {
    ...
    return $(".research-layout").on("click", "._methodology-link", function(e) {
      toPhase("question", e);
  ...
  reloadSourceSlot = function(slot, content) {
    ...
    return slot.reloadSlot((slot.data('cardLinkName')) + "?" + query);
  };
