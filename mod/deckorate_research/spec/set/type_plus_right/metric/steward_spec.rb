RSpec.describe Card::Set::TypePlusRight::Metric::Steward do
  ...
  describe "updating stewards" do
    it "starts with community added metrics" do
      expect(researched_card.record.verification).to eq(level(:unverified))
    ...
    it "updates researched (not wikirating) when one metric steward updated" do
      ...
      expect(researched_card.record.verification).to eq(level(:steward_verified))
      expect(wikirating_card.record.verification).to eq(level(:unverified))
    end
    ...
    it "updates researched and wikirating when all metric stewards updated" do
      ...
      expect(researched_card.record.verification).to eq(level(:steward_verified))
      expect(wikirating_card.record.verification).to eq(level(:steward_verified))
    end
  end
end
