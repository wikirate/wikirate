RSpec.describe Card::Set::TypePlusRight::Record::CheckedBy do
  ...
  describe "wikirating verification" do
    it "starts as a 1 (unverified)" do
      expect(wikirating_card.record.verification).to eq(level(:unverified))
    end
    ...
    context "when a dependee researched card's verification changes" do
      it "lowers to 0 (flagged) if flagged" do
        ...
        expect(researched_card.record.verification).to eq(level(:flagged))
        expect(wikirating_card.record.verification).to eq(level(:flagged))
      end

      it "stays 1 (unverified) if verified" do
        check_record researched_card
        expect(researched_card.record.verification).to eq(level(:community_verified))
        expect(wikirating_card.record.verification).to eq(level(:unverified))
      end
    end
    ...
    context "when all dependee research cards are verified" do
      it "raises to 2" do
        check_record researched_card
        check_record researched_card_2
        expect(researched_card.record.verification).to eq(level(:community_verified))
        expect(wikirating_card.record.verification).to eq(level(:community_verified))
      end
    end
  end
