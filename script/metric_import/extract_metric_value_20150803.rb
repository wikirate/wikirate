...
CSV.foreach(("script/metric_import/accepted_metric_20150803.csv"),:encoding => 'windows-1251:utf-8',:headers => true, :header_converters => :symbol, :converters => :all) do |row|
...
CSV.foreach(("script/metric_import/validness_of_companies_for_unglobal.csv"),:headers => true, :header_converters => :symbol, :converters => :all) do |row|
...
Card::Auth.as_bot do
  ...
  file_name = "script/metric_import/import_fixed.json"
  ...
  begin
    ...
    File.open("script/metric_import/metric_to_import_20150803.csv", 'w') { |file| file.write(result_str) }
