...
if ARGV.length == 0
  ...
  puts "EX: ruby script/import_metric_value.rb script/metric/csv/metric_to_import.csv".green
  exit
end
...
Card::Auth.as_bot do
  ...
  CSV.foreach(csv_file_path,:encoding => 'windows-1251:utf-8',:headers => true, :header_converters => :symbol, :converters => :all) do |row|
